//
//  CalfCircumferenceWorkerTests.swift
//  CGAssessmentTests
//
...
final class CalfCircumferenceWorkerTests: XCTestCase {
    ...
    func testSaveAssessmentProgress() {
        ...
        do {
            try worker.updateCalfCircumferenceProgress(with: .init(circumference: 32.34, isDone: false))
            currentExpectation?.fulfill()
        } catch {
            XCTFail("Test failed with error \(error.localizedDescription)")
        }
