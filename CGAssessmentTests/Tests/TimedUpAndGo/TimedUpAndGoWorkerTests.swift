...
final class TimedUpAndGoWorkerTests: XCTestCase {
    ...
    func testSaveAssessmentProgress() {
        ...
        do {
            try worker.updateTimedUpAndGoProgress(with: .init(typedElapsedTime: 4.87, elapsedTime: 8.64, selectedOption: .firstOption, isDone: false))
            currentExpectation?.fulfill()
        } catch {
            XCTFail("Test failed with error \(error.localizedDescription)")
        }
