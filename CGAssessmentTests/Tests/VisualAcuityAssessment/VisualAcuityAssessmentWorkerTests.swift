//
//  VisualAcuityAssessmentWorkerTests.swift
//  CGAssessmentTests
//
...
final class VisualAcuityAssessmentWorkerTests: XCTestCase {
    ...
    func testGetAssessmentProgress() {
        ...
        do {
            let test = try worker.getVisualAcuityAssessmentProgress()

            XCTAssertEqual(test?.selectedOption, 9)
    ...
    func testSaveAssessmentProgress() {
        ...
        do {
            try worker.updateVisualAcuityAssessmentProgress(with: .init(selectedOption: .secondOption, isDone: false))
            currentExpectation?.fulfill()
        } catch {
            XCTFail("Test failed with error \(error.localizedDescription)")
        }
