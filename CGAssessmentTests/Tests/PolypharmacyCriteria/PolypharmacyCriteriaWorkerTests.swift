//
//  PolypharmacyCriteriaWorkerTests.swift
//  CGAssessmentTests
//
...
final class PolypharmacyCriteriaWorkerTests: XCTestCase {
    ...
    func testGetAssessmentProgress() {
        ...
        do {
            let test = try worker.getPolypharmacyCriteriaProgress()

            XCTAssertEqual(test?.numberOfMedicines, 3)
    ...
    func testSaveAssessmentProgress() {
        ...
        do {
            try worker.updatePolypharmacyCriteriaProgress(with: .init(numberOfMedicines: 10, isDone: false))
            currentExpectation?.fulfill()
        } catch {
            XCTFail("Test failed with error \(error.localizedDescription)")
        }
